                           ...
                           const DXGI_PRESENT_PARAMETERS* pPresentParameters)
{
    ...
    if (willPresent && fgFeatureActive)
    {
        ...
        {
            XeFGTrace::Record(XeFGTrace::EventType::FSRFGPresentCallbackEnter, reinterpret_cast<uint64_t>(This),
                              reinterpret_cast<uint64_t>(fg), reinterpret_cast<uint64_t>(state.currentFGSwapchain),
                              fg->FrameCount(), 0, 0, 0, TraceFlags(), 1);
            ffxPresentCallback();
            XeFGTrace::Record(XeFGTrace::EventType::FSRFGPresentCallbackExit, reinterpret_cast<uint64_t>(This),
                              reinterpret_cast<uint64_t>(fg), reinterpret_cast<uint64_t>(state.currentFGSwapchain),
                              fg->FrameCount(), 0, 0, 0, TraceFlags(), 1);
        }
        else if (state.activeFgInput == FGInput::FSRFG30)
        {
            XeFGTrace::Record(XeFGTrace::EventType::FSRFGPresentCallbackEnter, reinterpret_cast<uint64_t>(This),
                              reinterpret_cast<uint64_t>(fg), reinterpret_cast<uint64_t>(state.currentFGSwapchain),
                              fg->FrameCount(), 0, 0, 0, TraceFlags(), 2);
