...
bool Sl_Inputs_Dx12::reportResource(const sl::ResourceTag& tag, ID3D12GraphicsCommandList* cmdBuffer, uint32_t frameId)
{
    ...
    else if (tag.type == sl::kBufferTypeUIColorAndAlpha)
    {
        ...
        fgOutput->GetInterpolationRect(interpolationAfterWidth, interpolationAfterHeight);

        LOG_INFO("[RES-DIAG][SL-UI] frame={} index={} UI={}x{} texture={}x{} extent={}x{} base={},{} "
                 "currentInterpolation={}x{} fallbackApplied={} interpolationAfter={}x{}",
                 frameId, res.frameIndex, res.width, res.height, desc.Width, desc.Height,
                 tag.extent ? tag.extent.width : 0, tag.extent ? tag.extent.height : 0,
                 tag.extent ? tag.extent.left : 0, tag.extent ? tag.extent.top : 0, width, height, fallbackApplied,
                 interpolationAfterWidth, interpolationAfterHeight);
